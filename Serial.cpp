...
void sendWeatherInfoToWU(PlotBotDevice *device)
{
    ...
    if (client.connect(WEATHERSERVER, 80)) 
    {
        ...
        sprintf(URL, "%s&soilmoisture=%d", URL, soilmoisturePercentage);
...
void printWeatherInfo()
{
    ...
    Serial.printf("soilmoisture: %d, ", soilmoisture);
    Serial.printlnf("soilmoisturePercentage: %d", soilmoisturePercentage);
    
    Serial.printf("\tluminous flux: %.6f, ", lux);
    Serial.printf("CH0 Level: %d, ", CH0Level);
    Serial.printlnf("CH1 Level: %d", CH1Level);
}